...
void CEnvWeather::Spawn(void)
{
	...
	if (m_useFog) {
		...
		m_fogStartDistHl = newMin;
		m_fogEndDistHl = newMax;
...
void CEnvWeather::UpdateFog() {
	...
	float spanDist = m_fogEndDistHl - m_fogStartDistHl;
	...
	for (int k = 0; k < FOG_LAYERS; k++) {
		...
		if (k % 2 == 1) {
			...
			SetFogSphereRadius(fog, m_fogStartDistHl + spanDist * t, colorFog);
		}
		else {
			// color layer
...
void CEnvWeather::PlayWeatherSounds(CBasePlayer* plr) {
	...
	for (int k = 0; k < 2; k++) {
		...
		if (snd->lastVol[pidx] == vol && gpGlobals->time - snd->lastMsg[pidx] < 1.0f) {
			continue;
		}

		snd->lastVol[pidx] = vol;
		snd->lastMsg[pidx] = gpGlobals->time;
	...
	if (g_fog_enabled && m_useFog) {
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++) {
			...
			if (plr->IsSevenKewpClient()) {
				// sevenkewp clients get a special network message for rendering real fog.
				incompatClientPlayers |= PLRBIT(i);
			}
		}
